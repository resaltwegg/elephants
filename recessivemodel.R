...
if (scenario==1) totquota[t] <- 0 # Scenario 1 means no hunting at all
  ...
  # -------------------------------------------------------------
  if (scenario == 2){
    nbulls[t] <- sum(n.LLm[35:maxageclasses.LLm+1,t])
    if(totquota.all[t]>=nbulls[t]) n.LLm[35:maxageclasses.LLm+1,t] <-0 # if fewer elephants than quota, then all are being finished off
    else n.LLm[35:maxageclasses.LLm+1,t]<-table(factor(sample(rep(35:maxageclasses.LLm+1,n.LLm[35:maxageclasses.LLm+1,t]), sum(n.LLm[35:maxageclasses.LLm+1,t])-totquota.all[t]), levels=35:maxageclasses.LLm+1))[]
    ...
    if(totquota[t]>=(sum(n.SSm[35:maxageclasses.SSm+1,t])+sum(n.LSm[35:maxageclasses.LSm+1,t]))) n.SSm[35:maxageclasses.SSm+1,t] <-n.LSm[35:maxageclasses.LSm+1,t] <-0 # if fewer elephants than quota, then all are being finished off
    else {
      pLS <- sum(n.LSm[35:maxageclasses.LSm+1,t])/(sum(n.SSm[35:maxageclasses.SSm+1,t])+sum(n.LSm[35:maxageclasses.LSm+1,t])) # proportion LS
      n.LSm[35:maxageclasses.LSm+1,t]<-table(factor(sample(rep(35:maxageclasses.LSm+1,n.LSm[35:maxageclasses.LSm+1,t]), sum(n.LSm[35:maxageclasses.LSm+1,t])-round(totquota[t]*pLS)), levels=35:maxageclasses.LSm+1))[]
      n.SSm[35:maxageclasses.SSm+1,t]<-table(factor(sample(rep(35:maxageclasses.SSm+1,n.SSm[35:maxageclasses.SSm+1,t]), sum(n.SSm[35:maxageclasses.SSm+1,t])-round(totquota[t]*(1-pLS))), levels=35:maxageclasses.SSm+1))[]
    }
  } # end scenario 2
  ...
  if (scenario == 3){
    ...
    nbulls[t] <- sum(n.LLm[21:34,t])
    if(totquota.all[t]>=nbulls[t]) n.LLm[21:34,t] <-0 # if fewer elephants than quota, then all are being finished off
    else n.LLm[21:34,t]<-table(factor(sample(rep(21:34,n.LLm[21:34,t]), sum(n.LLm[21:34,t])-totquota.all[t]), levels=21:34))[]
    
    killedbigtuskers[t] <- min(totquota.all[t], nbulls[t])
    totquota[t]<-totquota.all[t] - killedbigtuskers[t]  # works out how many small-tuskers will be killed to fill quota
    
    # hunting of small-tusked bulls >21 years if quota is not filled by big tuskers
    if(totquota[t]>=(sum(n.SSm[21:maxageclasses.SSm+1,t])+sum(n.LSm[21:maxageclasses.LSm+1,t]))) n.SSm[21:maxageclasses.SSm+1,t] <-n.LSm[21:maxageclasses.LSm+1,t] <-0 # if fewer elephants than quota, then all are being finished off
    else {
      pLS <- sum(n.LSm[21:maxageclasses.LSm+1,t])/(sum(n.SSm[21:maxageclasses.SSm+1,t])+sum(n.LSm[21:maxageclasses.LSm+1,t])) # proportion LS
      n.LSm[21:maxageclasses.LSm+1,t]<-table(factor(sample(rep(21:maxageclasses.LSm+1,n.LSm[21:maxageclasses.LSm+1,t]), sum(n.LSm[21:maxageclasses.LSm+1,t])-round(totquota[t]*pLS)), levels=21:maxageclasses.LSm+1))[]
      n.SSm[21:maxageclasses.SSm+1,t]<-table(factor(sample(rep(21:maxageclasses.SSm+1,n.SSm[21:maxageclasses.SSm+1,t]), sum(n.SSm[21:maxageclasses.SSm+1,t])-round(totquota[t]*(1-pLS))), levels=21:maxageclasses.SSm+1))[]
    }
  ...
  if (scenario == 4){
    ...
    nbulls21[t] <- sum(n.LLm[21:34,t])
    nbulls35[t] <- sum(n.LLm[35:maxageclasses.LLm+1,t])
    
    if(totquota.all[t]/2>=nbulls35[t]) n.LLm[35:maxageclasses.LLm+1,t] <-0 # if fewer elephants than quota, then all are being finished off
    else n.LLm[35:maxageclasses.LLm+1,t]<-table(factor(sample(rep(35:maxageclasses.LLm+1,n.LSm[35:maxageclasses.LLm+1,t]), sum(n.LLm[35:maxageclasses.LLm+1,t])-totquota.all[t]/2), levels=35:maxageclasses.LLm+1))[]
    
    if(totquota.all[t]/2>=nbulls21[t]) n.LLm[21:34,t] <-0 # if fewer elephants than quota, then all are being finished off
    else n.LLm[21:34,t]<-table(factor(sample(rep(21:34,n.LLm[21:34,t]), sum(n.LLm[21:34,t])-totquota.all[t]/2), levels=21:34))[]
  ...
  if (scenario == 5){
    ...
    nbulls[t] <- sum(n.LLm[35:maxageclasses.LLm+1,t])
    if(totquota.all[t]/2>=nbulls[t]) n.LLm[35:maxageclasses.LLm+1,t] <-0 # if fewer elephants than quota, then all are being finished off
    else n.LLm[35:maxageclasses.LLm+1,t]<-table(factor(sample(rep(35:maxageclasses.LLm+1,n.LLm[35:maxageclasses.LLm+1,t]), sum(n.LLm[35:maxageclasses.LLm+1,t])-totquota.all[t]/2), levels=35:maxageclasses.LLm+1))[]
  ...
  if (scenario == 6){
    ...
    nbulls[t] <- sum(n.LLm[50:maxageclasses.LLm+1,t])
    if(totquota.all[t]/2>=nbulls[t]) n.LLm[50:maxageclasses.LLm+1,t] <-0 # if fewer elephants than quota, then all are being finished off
    else n.LLm[50:maxageclasses.LLm+1,t]<-table(factor(sample(rep(50:maxageclasses.LLm+1,n.LSm[50:maxageclasses.LLm+1,t]), sum(n.LLm[50:maxageclasses.LLm+1,t])-totquota.all[t]/2), levels=50:maxageclasses.LLm+1))[]
    
    killedbigtuskers[t] <- min(totquota.all[t]/2, nbulls[t])
    totquota[t]<-totquota.all[t] - killedbigtuskers[t]  # works out how many small-tuskers will be killed to fill quota
    
    # hunting of small-tusked bulls >50 
    if(totquota[t]>=(sum(n.SSm[50:maxageclasses.SSm+1,t])+sum(n.LSm[50:maxageclasses.LSm+1,t]))) n.SSm[50:maxageclasses.SSm+1,t] <-n.LSm[50:maxageclasses.LSm+1,t] <-0 # if fewer elephants than quota, then all are being finished off
    else {
      pLS <- sum(n.LSm[50:maxageclasses.LSm+1,t])/(sum(n.SSm[50:maxageclasses.SSm+1,t])+sum(n.LSm[50:maxageclasses.LSm+1,t])) # proportion LS
      n.LSm[50:maxageclasses.LSm+1,t]<-table(factor(sample(rep(50:maxageclasses.LSm+1,n.LSm[50:maxageclasses.LSm+1,t]), sum(n.LSm[50:maxageclasses.LSm+1,t])-round(totquota[t]*pLS)), levels=50:maxageclasses.LSm+1))[]
      n.SSm[50:maxageclasses.SSm+1,t]<-table(factor(sample(rep(50:maxageclasses.SSm+1,n.SSm[50:maxageclasses.SSm+1,t]), sum(n.SSm[50:maxageclasses.SSm+1,t])-round(totquota[t]*(1-pLS))), levels=50:maxageclasses.SSm+1))[]
    }
...
bigtuskers[j] <- sum(n.LLm[,nyears])
oldbigtuskers[j] <- n.LLm[maxageclasses.LLm+1,nyears]
